...
app = FastAPI(title="DhanScreen API", version="3.1")
...
# ── Verified Dhan NSE_EQ security IDs (v3.1 — corrected) ─────────────────────
# Cross-checked against Dhan api-scrip-master.csv NSE_EQ segment
# Fixes: ABBOTINDIA, ABCAPITAL, ANGELONE, APLAPOLLO, BANKBARODA and ~55 others
VERIFIED_IDS = {
    # A
    "ABBOTINDIA":   "4717",
    "ABCAPITAL":    "11195",
...
def load_symbols_disk():
    global SYMBOLS
    try:
        with open(DISK_PATH) as f: d = json.load(f)
        if len(d.get("symbols",[])) >= 100:
            SYMBOLS = d["symbols"]
            cache["symbol_source"] = "disk"
            print(f"[sym] disk: {len(SYMBOLS)} symbols")
            return True
    except: pass
    return False
...
def ensure_symbols(tok=""):
    ...
    # Try CSV first — has the most accurate/fresh IDs
    if tok and load_symbols_csv(tok): return True
    if load_symbols_disk(): return True
    load_symbols_verified()   # always works
    return True


def _upgrade_ids_background(tok):
    """Try CSV. If that fails, fix wrong IDs via marketfeed scan."""
...
def fix_missing_ids(tok):
    """
    Scan Dhan NSE_EQ IDs to find correct security_id for missing symbols.
...
def run_screener(tok):
    ...
    for sym in SYMBOLS:
        try:
            ...
            chg_pct, prev_close, momentum5d, vol_ratio, avg_vol7d = \
                compute_metrics(q, hist, market_open, today_str)
            results.append({
    ...
    # Trigger ID fix in background if still too many missing
...
async def startup():
    def _boot():
        ...
        # FIX v3.1: Try CSV first on boot (gets accurate IDs from Dhan directly)
        # Falls back to disk cache, then hardcoded verified IDs
        csv_ok = load_symbols_csv(tok) if tok else False
        if not csv_ok:
            loaded = load_symbols_disk()
            if not loaded:
                load_symbols_verified()
                # If CSV failed, schedule a background ID scan to self-correct
                if tok:
                    threading.Thread(target=fix_missing_ids, args=(tok,), daemon=True).start()
...
def debug_missing():
    """Show exactly which symbols have no quote and their current security IDs."""
    missing = cache.get("debug", {}).get("missing_sample", [])
    skipped = cache.get("skipped", [])
    details = []
    sym_map  = {s["symbol"]: s["security_id"] for s in SYMBOLS}
    for sym in missing:
        details.append({"symbol": sym, "security_id": sym_map.get(sym, "?")})
    return {
        "missing_count": cache.get("debug", {}).get("missing_count", 0),
        "missing_details": details,
        "skipped_sample": skipped[:20],
        "symbol_source": cache["symbol_source"],
        "tip": "If security_id looks wrong, hit POST /api/symbols/refresh to reload from Dhan CSV"
    }
