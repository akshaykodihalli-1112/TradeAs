...
# ---------------------------------------------------------------------------
# SEED LIST — all 185 NSE FNO stocks with verified security IDs
# Used as fallback if Dhan master CSV is unreachable
# ---------------------------------------------------------------------------
SEED_SYMBOLS = [
...
# ---------------------------------------------------------------------------
# Fetch live FNO symbol list from Dhan master CSV
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dhan historical data — with retry + backoff
# ---------------------------------------------------------------------------
    ...
    for attempt in range(retries):
        try:
            ...
            if resp.status_code == 400:
                print(f"  Bad request (400) for security_id={security_id}: {resp.text[:120]}")
                return None

            if resp.status_code == 401:
                print("  Unauthorized (401) — token expired. Regenerate from Dhan portal.")
                return None
            ...
            print(f"  Timeout for security_id={security_id}, attempt {attempt + 1}")
            if attempt < retries - 1:
                time.sleep(1)

        except requests.exceptions.RequestException as e:
            print(f"  Request error for security_id={security_id}: {e}")
            if attempt < retries - 1:
                time.sleep(1)
            else:
                return None
...
def fetch_screener(client_id=None, access_token=None):
    ...
    if not cid or not tok:
        ...
        print("Screener skipped: no credentials set.")
        return
    ...
    total = len(SYMBOLS)
    print(f"Starting screener for {total} stocks...")
    ...
    for i, sym in enumerate(SYMBOLS):
        ...
        # Rate limiting: 0.35s base + 2s pause every 15 stocks
        time.sleep(0.35)
        if i > 0 and i % 15 == 0:
            print(f"  Progress: {i}/{total} done, pausing 2s...")
            time.sleep(2)
    ...
    print(f"Refreshed {cache['updated_at']} — {len(results)} stocks OK, {len(errors)} errors")
...
async def startup():
    ...
    if CREDS["client_id"] and CREDS["access_token"]:
        ...
        print("No env credentials. Pass x-client-id + x-access-token headers to /api/screener.")
...
def ping_dhan(
    ...
    """Test Dhan connectivity — call this first to verify your token."""
...
def get_screener(
    ...
    """Returns cached screener data. Pass credentials to trigger a fresh fetch."""
...
def momentum(min_pct: float = 3.0):
    """Stocks with 5-day momentum >= min_pct%"""
...
def volume_spike(min_ratio: float = 2.0):
    """Stocks where today's volume >= min_ratio x 7-day average"""
...
def strong(min_mom: float = 5.0, min_vol: float = 2.5):
    """Stocks with both strong momentum AND volume spike"""
    return {
        "data": [
...
def top(limit: int = 20):
    """Top N stocks by volume ratio"""
    return {"data": cache["data"][:limit]}


@app.get("/api/symbols")
def list_symbols():
    """List all symbols currently loaded"""
    return {"count": len(SYMBOLS), "symbols": SYMBOLS}


@app.get("/api/health")
def health():
    """Service health and last refresh info"""
    return {
        "status":        "ok",
